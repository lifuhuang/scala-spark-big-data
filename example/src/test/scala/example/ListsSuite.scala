 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test( testName = "sum of a few numbers (including negative ones)") {
    assert(sum(List(-5, 3, 2, -10)) === -10)
  }

  test( testName = "max of a few numbers (including negative ones)") {
    assert(max(List(-5, 3, 2, -10)) === 3)
  }

  test( testName = "sum of a few numbers (only negative ones") {
    assert(sum(List(-1, -10, -3)) === -14)
  }

  test( testName = "max of a few numbers (only negative ones") {
    assert(max(List(-1, -10, -3)) === -1)
  }

  test( testName = "sum of an empty list") {
    assert(sum(List()) === 0)
  }

  test( testName = "max of an empty list") {
